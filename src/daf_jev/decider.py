...
- ``client_error``  — the client factory or default client construction
  raised (latched for the Decider's lifetime)
- ``latched``       — ``max_consecutive_failures`` consecutive failures
  reached; every later decide short-circuits with this reason
...
__all__ = ["Budget", "ConfidenceGate", "DecisionEvent", "Decider"]
...
class DecisionEvent:
    ...
    are None exactly when ``source == "model"``. ``latency_s`` is the wall
    time of the whole ``decide()`` call, measured with the injected clock.
    """
...
class ConfidenceGate:
    ...
    def __call__(self, answers: Mapping[str, Answer]) -> Optional[str]:
...
class Budget:
    ...
    At least one threshold is required. ``exceeded()`` returns a
    human-readable reason when any limit is met or passed, else None; the
    Decider checks it before every ask and charges ``attempts`` once per
    ...
    def exceeded(self, usage: UsageSnapshot) -> Optional[str]:
...
class _ResolutionFailed(Exception):
    ...
    def __init__(self, reason: str, error: Optional[str]) -> None:
...
class Decider(Generic[S, T]):
    ...
    def __init__(
        ...
        *,
        ...
        gate: Optional[Callable[[Mapping[str, Answer]], Optional[str]]] = None,
        should_ask: Optional[Callable[[S], bool]] = None,
        budget: Optional[Budget] = None,
        cache: Optional[MutableMapping[Any, T]] = None,
        cache_key: Optional[Callable[[S], Any]] = None,
        ...
        self._latch_error: Optional[str] = None
        self.last_event: Optional[DecisionEvent] = None
    ...
    def _decide(self, state: S, t0: float) -> T:
        # 1. Cache hit: return the cached action without asking.
        ...
        try:
            if self._breaker is not None:
                response = self._breaker.call(_ask)
            else:
                response = _ask()
        ...
        # 10. Cache the action and emit the model event.
        if self._cache is not None:
            self._cache[self._cache_key(state)] = action
    ...
    def _note_failure(self, error: Optional[str]) -> None:
    ...
    def _latch(self, reason: str, error: Optional[str]) -> None:
        """Latch the decider: every later decide short-circuits with ``reason``."""
        self._latched = True
        self._latch_reason = reason
        self._latch_error = error

    def _fallback(
        self, state: S, reason: Optional[str], error: Optional[str], t0: float
    ) -> T:
    ...
    def _emit(
        ...
        if self._on_event is not None:
            try:
                ...
                pass  # diagnostics hooks must never break the decision loop
